...
export default function RegisterPage() {
  ...
  // Pre-fill name/email from Clerk user in OAuth mode
  useEffect(() => {
    if (isOAuthMode && clerkUser) {
  ...
  const formValid = isOAuthMode
    ? name.trim().length > 0 && isValidPhone && institutionIds.length > 0
    : name.trim().length > 0 &&
      isValidEmail &&
      isValidPhone &&
  ...
  return (
    <AuthLayout
      title={isOAuthMode ? "Complete seu cadastro" : "Crie sua conta"}
      subtitle={isOAuthMode ? "Preencha os dados restantes para finalizar." : "Comece seu perfil com instituicoes e materias."}
    >
      {!isOAuthMode && (
        <>
          <button
      ...
      <form className="space-y-5" onSubmit={handleSubmit}>
        ...
        <button
          ...
          className="w-full bg-[#ea580c] text-white font-bold text-xs uppercase tracking-[0.14em] py-3.5 rounded-md hover:bg-[#c2410c] active:scale-[0.99] transition-all duration-150 disabled:opacity-50"
        >
          {loading ? (isOAuthMode ? "Finalizando..." : "Criando conta...") : (isOAuthMode ? "Finalizar cadastro" : "Criar conta")}
        </button>
      </form>
