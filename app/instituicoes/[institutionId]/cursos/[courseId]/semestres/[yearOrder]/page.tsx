...
import { useSemesterSubjects } from "@/lib/queries/institutions";
import { SubjectIcon } from "@/components/SubjectIcon";
import { BackLink } from "@/components/BackLink";

type PageProps = {
  params: Promise<{ institutionId: string; courseId: string; yearOrder: string }>;
};

export default function SemesterSubjectsPage({ params }: PageProps) {
  const { institutionId, courseId, yearOrder: yearOrderStr } = use(params);
  const yearOrder = parseInt(yearOrderStr, 10);
  const { data, isLoading } = useSemesterSubjects(institutionId, courseId, yearOrder);
  ...
  if (!data) {
    return <div className="p-8 text-muted-foreground">Semestre nao encontrado.</div>;
  }

  const { institution, course, yearLabel, subjects } = data;

  return (
    <div className="space-y-14">
      <header className="space-y-6">
        <BackLink
          href={`/instituicoes/${institutionId}/cursos/${courseId}`}
          label={course.name}
        />
      ...
      {subjects.length === 0 ? (
        <div className="rounded-sm border border-border bg-surface p-12 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Este semestre ainda nao possui materias disponiveis.
          </p>
        </div>
      ) : (
        <SubjectGrid
